package day06_a_arithmetic_operators;

public class TypeCasting {
    public static void main(String[] args) {
        byte b = 40;
        int i = b; //implicit casting or widening casting - converting byte in to int

        int i2 = 100;
        //byte b2 = i2; cannot assign implicitly/automaticly/ directy
        byte b2 = (byte)i2; //Explicit casting / narrowing casting = converting int to byte
        System.out.println(i2);
        System.out.println(b2);
        System.out.println();

        int i3 = 150;
        byte b3 = (byte)i3;
        System.out.println(i3);
        System.out.println(b3);

        int i4 = 500;
        long l4 = i4; // IMPLICIT CASTING / WIDENING CASTING - AUTOMATICALLY: converting int into long | long > int
        System.out.println(i4);
        System.out.println(l4);
        System.out.println();


        long l5 = 500;
        // int i5 = l5;  // long > int ---> so explicit casting is needed
        int i5 = (int)l5; // EXPLICIT CASTING / NARROWING CASTING
        System.out.println(l5);
        System.out.println(i5);


        System.out.println("-------------------");
        int i6 = 50;
        double d6 = i6;  // IMPLICIT CASTING /WIDENING CASTING
        System.out.println(i6);
        System.out.println(d6);
        System.out.println();

        double d7 = 50.0;
        //int i7 = d7;
        int i7 = (int)d7; // EXPLICIT CASTING / NARROWING CASTING
        System.out.println(d7);
        System.out.println(i7);
        System.out.println();


        double d8 = 50.97;
        //int i8 = d8;
        int i8 = (int)d8; // EXPLICIT CASTING / NARROWING CASTING -  portion of the data will be lost
        System.out.println(d8);
        System.out.println(i8);

    }
}
